...
namespace Engine::Input {
  ...
  void Keyboard::processInput() {
    ...
    auto processKey = [&](int keyCode, KeyboardEvent &event) {
      int state = glfwGetKey(window, keyCode);
      bool isPressed = state == GLFW_PRESS;

      // Get previous state
      bool wasPressed = previousKeyStates[keyCode];

      int action = -1; // Initialize action to -1
      ...
      if (action != -1) {
        KeyboardContext context = {
          ...
          .held = action == GLFW_REPEAT,
          .released = action == GLFW_RELEASE,
          .shiftPressed = (mods & GLFW_MOD_SHIFT) != 0,
          .controlPressed = (mods & GLFW_MOD_CONTROL) != 0,
          .altPressed = (mods & GLFW_MOD_ALT) != 0,
          .superPressed = (mods & GLFW_MOD_SUPER) != 0,
          .capsLockPressed = (mods & GLFW_MOD_CAPS_LOCK) != 0,
          .numLockPressed = (mods & GLFW_MOD_NUM_LOCK) != 0,
        };
