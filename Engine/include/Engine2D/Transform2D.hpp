...
namespace Engine2D {
  ...
  class Transform2D final : public Component2D {
    ...
    public:
      ...
      std::vector<Engine::Ptr<Entity2D>>::iterator begin();
      std::vector<Engine::Ptr<Entity2D>>::iterator end();
      [[nodiscard]] std::vector<Engine::Ptr<Entity2D>>::const_iterator begin() const;
      [[nodiscard]] std::vector<Engine::Ptr<Entity2D>>::const_iterator end() const;
      ...
      /// @returns The rotation of the Entity2D this transform is attached to in world coordinates in **radians**
      ...
      [[nodiscard]] const std::vector<Engine::Ptr<Entity2D>> &Children() const;
    ...
    private:
      ...
      /// Rotation of the transform in degrees in local space.
      ENGINE_SERIALIZE float rotation;
      /// Rotation of the transform in radians in world space.
      ...
      Engine::Ptr<Entity2D> parent;
      /// The list of all the children of this entity
      std::vector<Engine::Ptr<Entity2D>> children;
       ...
       */
      Transform2D(
        glm::vec2 position, float rotation, glm::vec2 scale, const Engine::Ptr<Entity2D> &entity,
      ...
      void addChild(const Engine::Ptr<Entity2D> &child);
      /// Removes the given child to the child list of the current entity's transform
      void removeChild(const Engine::Ptr<Entity2D> &child);
