...
template<> struct std::hash<Engine2D::Physics::ContactPair> {
  size_t operator()(const Engine2D::Physics::ContactPair &pair) const noexcept {
    const auto hash1 = std::hash<std::shared_ptr<Engine2D::Physics::Collider2D> >{}(pair.collider1);
    const auto hash2 = std::hash<std::shared_ptr<Engine2D::Physics::Collider2D> >{}(pair.collider2);
    const auto hash3 = std::hash<std::shared_ptr<Engine2D::Physics::Rigidbody2D> >{}(pair.rigidbody1);
    const auto hash4 = std::hash<std::shared_ptr<Engine2D::Physics::Rigidbody2D> >{}(pair.rigidbody2);
